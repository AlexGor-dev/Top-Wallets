...
namespace Complex.Ton
{
    public static class JettonController
    {
        ...
        public static JettonDeployData CreateJetton(long queryId, string owner, JettonDeployInfo info, string offchainUri)
        {
            ...
            return new JettonDeployData(owner, info, offchainUri, queryId, jettonMinterAddress, jettonWalletAddress, deployParams);
        }
        ...
        public static MessageData CreateChangeContent(string jettonMinterAddress, long queryId, JettonDeployInfo info, string offchainUri)
        {
        ...
        private static JettonInfo JettonFrommOffChain(Slice cs, string jtonAddress, string owner, UInt128 totalSupply)
        {
            ...
            try
            {
                ...
                if (array != null && array.Count > 0)
                {
                    ...
                    return new JettonInfo(array.GetString("name"), array.GetString("description"), symbol, image, new Balance(symbol, totalSupply, desimals, 3), desimals, jtonAddress, owner, array.GetString("color"), array.GetString("deployer"));
                }
            }
            catch (Exception e)
            {
        ...
        private static Cell BuildJettonOnchainMetadata(JettonDeployInfo info)
        {
        ...
        private static Cell JettonInitData(string owner, JettonDeployInfo info, string offchainUri)
        {
            return CellBuilder.Begin()
                .StoreCoins(0)
                .StoreAddress(owner)
        ...
        private static ContractDeployData CreateDeployParams(long queryId, string owner, JettonDeployInfo info, string offchainUri)
        {
            return new ContractDeployData(owner, null, ContractController.GetStateInit(Cell.FromBase64Boc(JettonMinterCode), JettonInitData(owner, info, offchainUri)), deployGas, Mint(queryId, owner, info.TotalSupply, mintGas));
        }
    }
}
