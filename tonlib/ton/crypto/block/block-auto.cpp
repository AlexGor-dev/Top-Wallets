...
bool ConfigParam::unpack(vm::CellSlice& cs, ConfigParam::Record_cons80& data) const {
  return t_JettonBridgeParams.fetch_to(cs, data.x)
      && m_ == 80;
}

bool ConfigParam::unpack_cons80(vm::CellSlice& cs, Ref<CellSlice>& x) const {
  return t_JettonBridgeParams.fetch_to(cs, x)
      && m_ == 80;
}

bool ConfigParam::cell_unpack(Ref<vm::Cell> cell_ref, ConfigParam::Record_cons80& data) const {
  if (cell_ref.is_null()) { return false; }
  auto cs = load_cell_slice(std::move(cell_ref));
  return unpack(cs, data) && cs.empty_ext();
}

bool ConfigParam::cell_unpack_cons80(Ref<vm::Cell> cell_ref, Ref<CellSlice>& x) const {
  if (cell_ref.is_null()) { return false; }
  auto cs = load_cell_slice(std::move(cell_ref));
  return unpack_cons80(cs, x) && cs.empty_ext();
}
...
bool ConfigParam::pack(vm::CellBuilder& cb, const ConfigParam::Record_cons80& data) const {
  return t_JettonBridgeParams.store_from(cb, data.x)
      && m_ == 80;
}

bool ConfigParam::pack_cons80(vm::CellBuilder& cb, Ref<CellSlice> x) const {
  return t_JettonBridgeParams.store_from(cb, x)
      && m_ == 80;
}

bool ConfigParam::cell_pack(Ref<vm::Cell>& cell_ref, const ConfigParam::Record_cons80& data) const {
  vm::CellBuilder cb;
  return pack(cb, data) && std::move(cb).finalize_to(cell_ref);
}

bool ConfigParam::cell_pack_cons80(Ref<vm::Cell>& cell_ref, Ref<CellSlice> x) const {
  vm::CellBuilder cb;
  return pack_cons80(cb, std::move(x)) && std::move(cb).finalize_to(cell_ref);
}
...
const RefT t_Ref_TYPE_1615{t_Transaction_aux};
...
const RefT t_Ref_TYPE_1627{t_TrComputePhase_aux};
...
const RefT t_Ref_TYPE_1641{t_ShardStateUnsplit_aux};
...
const RefT t_Ref_TYPE_1652{t_ValueFlow_aux};
const RefT t_Ref_TYPE_1653{t_ValueFlow_aux1};
const NatWidth t_natwidth_3{3};
const RefT t_Ref_TYPE_1657{t_ShardDescr_aux};
...
const RefT t_Ref_TYPE_1671{t_McStateExtra_aux};
...
const RefT t_Ref_TYPE_1679{t_McBlockExtra_aux};
...
const RefT t_Ref_TYPE_1725{t_VmGasLimits_aux};
