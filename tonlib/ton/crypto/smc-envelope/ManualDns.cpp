...
td::Ref<ManualDns> ManualDns::create(td::Ref<vm::Cell> data, int revision) {
  return td::Ref<ManualDns>(
      true, State{ton::SmartContractCode::get_code(ton::WalletType::ManualDns, revision), std::move(data)});
}
...
td::optional<td::int32> ManualDns::guess_revision(const vm::Cell::Hash& code_hash) {
  for (auto i : ton::SmartContractCode::get_revisions(ton::WalletType::ManualDns)) {
    if (ton::SmartContractCode::get_code(ton::WalletType::ManualDns, i)->get_hash() == code_hash) {
      return i;
    }
  }
  return {};
